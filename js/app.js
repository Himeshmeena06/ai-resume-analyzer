...
function setupApp() {
  ...
  if (saveApiKeyBtn) {
    saveApiKeyBtn.addEventListener("click", () => {
      ...
      if (key && !key.startsWith("••••")) {
        ...
        const originalText = saveApiKeyBtn.textContent.trim();
        ...
        const originalText = saveApiKeyBtn.textContent.trim();
        saveApiKeyBtn.textContent = "Enter a key!";
        saveApiKeyBtn.classList.add("bg-red-600");
        saveApiKeyBtn.classList.remove("bg-purple-600");
        setTimeout(() => {
          saveApiKeyBtn.textContent = originalText;
          saveApiKeyBtn.classList.remove("bg-red-600");
          saveApiKeyBtn.classList.add("bg-purple-600");
        }, 2000);
      }
    });
  }
  ...
  if (tabAnalyzer && tabBuilder && analyzerViewWrapper && builderViewWrapper) {
    tabAnalyzer.addEventListener("click", () => {
      ...
      tabAnalyzer.className = "px-5 py-2.5 rounded-xl text-sm font-semibold flex items-center gap-2 cursor-pointer transition-all duration-300 bg-gradient-to-r from-purple-600 to-indigo-600 text-white shadow-md shadow-purple-500/10 focus:outline-none interactive-element";
      // Set inactive styles for tabBuilder
      tabBuilder.className = "px-5 py-2.5 rounded-xl text-sm font-semibold flex items-center gap-2 cursor-pointer transition-all duration-300 text-slate-400 hover:text-slate-200 hover:bg-slate-800/40 focus:outline-none interactive-element";
    ...
    tabBuilder.addEventListener("click", () => {
      ...
      tabBuilder.className = "px-5 py-2.5 rounded-xl text-sm font-semibold flex items-center gap-2 cursor-pointer transition-all duration-300 bg-gradient-to-r from-purple-600 to-indigo-600 text-white shadow-md shadow-purple-500/10 focus:outline-none interactive-element";
      // Set inactive styles for tabAnalyzer
      tabAnalyzer.className = "px-5 py-2.5 rounded-xl text-sm font-semibold flex items-center gap-2 cursor-pointer transition-all duration-300 text-slate-400 hover:text-slate-200 hover:bg-slate-800/40 focus:outline-none interactive-element";
      ...
      try {
        ...
        if (!contentType.includes("application/json")) {
          ...
          if (rawText.trim().startsWith("<") || rawText.includes("The page c") || rawText.includes("404")) {
            ...
            throw new Error("This website is running on Vercel, so please enter your own Gemini API Key at the top to analyze your resume.");
          }
        }
      ...
      if (!resumeData) {
        try {
          ...
          if (!contentType.includes("application/json")) {
            ...
            if (rawText.trim().startsWith("<") || rawText.includes("The page c") || rawText.includes("404")) {
              showStaticHostingWarning();
              throw new Error("This website is running on Vercel, so please enter your own Gemini API Key at the top to build your resume.");
            }
          }
